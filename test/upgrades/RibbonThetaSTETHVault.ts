...
describe("RibbonThetaSTETHVault upgrade", () => {
  ...
  before(async function () {
    ...
    const deploymentNames = ["RibbonThetaVaultSTETHCall"];
    deploymentNames.forEach((name) => vaults.push(deployments.mainnet[name]));
  });

  checkIfStorageNotCorrupted(deployments.mainnet.RibbonThetaVaultSTETHCall);
});
...
function checkIfStorageNotCorrupted(vaultAddress: string) {
  ...
  describe(`Vault ${vaultAddress}`, () => {
    ...
    before(async () => {
      ...
      vault = await ethers.getContractAt("RibbonThetaSTETHVault", vaultAddress);
