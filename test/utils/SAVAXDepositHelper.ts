...
import { BLOCK_NUMBER, CHAINID, SAVAX_ADDRESS } from "../../constants/constants";
...
describe("SAVAXDepositHelper", () => {
  ...
  before(async () => {
    ...
    const SAVAXDepositHelper = await ethers.getContractFactory("SAVAXDepositHelper");
  ...
  it("Stakes AVAX and deposits sAVAX into vault", async () => {
    ...
    assert.equal(
      (await sAVAXVault.depositReceipts(signer.address))[AMOUNT_INDEX].toString(),
      "0"
    );
